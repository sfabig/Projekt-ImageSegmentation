...
if __name__ == '__main__':
    ...
    # Load model
    model_name = '../trained_prostate_model.hdf5'
    ...
    plt.tight_layout()
    plt.savefig('results_prostata.png')
    plt.show()

    with open('./test_scores.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['Test File', 'Intersection over Union', 'Dice Coefficient'])
        for i in range(n_test):
            writer.writerow([df.iloc[i][0], ious[i], dices[i]])
